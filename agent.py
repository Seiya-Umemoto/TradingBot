...
class Agent:
    ...
    def decide_action(self, pred_value, pred_policy, epsilon):
        ...
        pred = pred_policy # 정책이 우선됨
        if pred is None:
            pred = pred_value # Q Value로 결정
        ...
        # 탐험 결정
        if np.random.rand() < epsilon: # 엡실론이 1이면 무조건 이쪽
            exploration = True
            if np.random.rand() < self.exploration_base: # 무조건 매수
    ...
    def decide_trading_unit(self, confidence):
        ...
        added_traiding = max(min( # 최대 매수/매도 수를 넘지 않도록
    ...
    def act(self, action, confidence):
        if not self.validate_action(action): # 유효하지 않으면 관망
        ...
        self.profitloss = (
            (self.portfolio_value - self.initial_balance) \
                / self.initial_balance # 초기 자본 기준 수익률
        )
